...
const c={Date,ADMIN_INITIAL_LOAD_SLOW_MS:12000,adminAuthBlocksDataLoad:false,document:{querySelector:()=>({setAttribute(){}})},window:{setTimeout,clearTimeout},
 ...
 fetchAdminOrdersFromGas:()=>new Promise(r=>orderResolve=r),renderAdminOrders:()=>{events.push('render');return true},updateStatsCounters(){},handleBatchCheckChange(){},updateNotifyButton(){},applyReadOnlyModeToRealOrders(){},markAdminSessionVerified(){},recordAdminReadBreadcrumb(){},showAdminAuthOverlay(){},hideAdminAuthOverlay:()=>events.push('usable'),restoreAdminTab:()=>events.push('restore-tab'),reconcilePendingAdminShipment:async()=>events.push('readback'),recoverPendingAdminCreateOrderOnLoad:async()=>{},fetchAdminProductCatalogFromGas:()=>{throw Error('catalog must not run on the initial orders critical path')}};
