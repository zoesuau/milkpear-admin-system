...
// COD collection is a separate admin override; automatic mode remains server-calculated.
f=fixture();f.node('new-payment-method').value='cod';f.node('new-cod-amount-mode').value='manual';f.node('new-cod-amount').value='1200';await f.c.handleNewOrderSubmit(event);assert.equal(f.requests[0].codCollectionAmount,1200);
f=fixture();f.node('new-payment-method').value='cod';f.node('new-cod-amount-mode').value='manual';f.node('new-cod-amount').value='0';await f.c.handleNewOrderSubmit(event);assert.equal(f.requests[0].codCollectionAmount,0);
for(const bad of ['', '-1', '1.5', 'abc']) {f=fixture();f.node('new-payment-method').value='cod';f.node('new-cod-amount-mode').value='manual';f.node('new-cod-amount').value=bad;await f.c.handleNewOrderSubmit(event);assert.equal(f.requests.length,0);assert.match(f.node('new-order-feedback').innerText,/代收金額/);}
f=fixture();f.node('new-payment-method').value='cod';f.node('new-cod-amount-mode').value='auto';f.node('new-cod-amount').value='9999';await f.c.handleNewOrderSubmit(event);assert.equal(Object.hasOwn(f.requests[0],'codCollectionAmount'),false);
f=fixture();f.node('new-payment-method').value='bank_unpaid';f.node('new-cod-amount-mode').value='manual';f.node('new-cod-amount').value='1200';await f.c.handleNewOrderSubmit(event);assert.equal(Object.hasOwn(f.requests[0],'codCollectionAmount'),false);
f=fixture();f.node('addOrderModal').classList.add('open');f.node('new-payment-method').value='cod';f.node('new-cod-amount-mode').value='manual';f.node('new-cod-amount').value='1200';f.c.persistAdminCreateDraft();const codDraft=f.c.readAdminCreateDraft();f.node('new-cod-amount-mode').value='auto';f.node('new-cod-amount').value='';f.c.restoreAdminCreateDraftFields(codDraft);assert.equal(f.node('new-cod-amount').value,'1200');assert.equal(f.node('new-cod-amount-mode').value,'manual');
console.log('COD submit and draft continuity: PASS');
